...
export CHECKOUTS=${SCRPIPT_HOME}/checkouts

QUICKSTART1=${HOME}/.quickstart1
QUICKSTART2=${HOME}/.quickstart2

: ${CMDS:="cleanup run_undercloud run_overcloud build_hosts"}
...
EOF
    ...
    export OPTS="${N_FLAG} -R ${RELEASE}  --config ${QUICKSTART_CONFIG} --nodes config/nodes/3ctlr_1comp.yml -e undercloud_disk=250  -e   control_memory=8192 -e undercloud_undercloud_nameservers=10.5.30.160 -e enable_pacemaker=true -e enable_port_forward_for_tripleo_ui=false -e tripleo_ui_secure_access=false"
...
run_overcloud() {
    ${QUICKSTART} ${OPTS} ${OVERCLOUD_ONLY} ${SKIP_TAGS} ${STACK_ARGS} 127.0.0.2
}

build_hosts() {
   cat /dev/null > ${HOSTS}
   grep ${QUICKSTART1}/hosts -e "^stack1.*ansible" >> ${HOSTS}
   grep ${QUICKSTART1}/hosts -e "^stack2.*ansible" >> ${HOSTS}
